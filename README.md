...
|----------|-------------------|
| **Linux** | `sudo apt-get install libasound2-dev` (ALSA headers for audio capture) |
| **macOS** | None (CoreAudio is built-in) |
| **Windows** | None (WASAPI is built-in) |

See [CONTRIBUTING.md](CONTRIBUTING.md) for development setup and [PROTOCOL.md](PROTOCOL.md) for the wire protocol specification.
